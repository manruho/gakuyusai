...
* 商品選択中はAPIに書き込まない
* 通常販売は「会計へ進む」で3分間有効な会計待ち注文と4文字IDを作成し、商品在庫を仮押さえする
* 会計待ち注文は受取画面へ即時通知し、薄い紺色で表示する。この段階では受取済みにできない
* 「お会計確定」で会計待ち注文を売上へ変換し、同じIDを黒色表示へ切り替える
* 商品選択へ戻る操作または3分の期限切れでは仮押さえを解放し、受取画面に赤字・取消線で表示する
* 前売り販売は従来どおり「前売り券を発行」で初めてAPIへ書き込む
* 二重送信を防ぐ
* 在庫不足時は完了画面に進ませない
...
* 設定画面で指定された販売モードを表示
* レジ1〜3は設定画面でそれぞれ通常販売／前売り販売専用を切り替えられる。初期状態は通常販売
* レジ4は常に前売り販売専用で、設定画面から変更できない
* 前売り販売の受取先は販売レジ番号にかかわらず受取4に固定し、通常販売は販売レジと同じ番号の受取先に送る
* 商品ボタン一覧
* カート
* 合計金額
* 預かり金額
...
事前販売:
  ...
  販売操作は前売り専用に設定されたレジ（レジ4を含む）に限定する
  前売り販売数は商品ごとに `floor(initial_stock * 0.3)` 個までとし、通常販売数や補充数で上限を変えない
  前売り上限に達した商品はレジで「前売り終了」と表示し、選択できないようにする
  受取開始日を過ぎた未使用の前売り券も、期限なく受取4の一覧・検索に表示する
```
...
12時分の一括補充は管理者がボタンを押したときだけ実行する。同じ営業日に複数回押しても在庫は一度だけ増やす。おにぎり16種へ順に `35, 30, 20, 30, 50, 10, 15, 25, 30, 30, 40, 35, 50, 30, 25, 10` 個、唐揚げへ200個を追加し、合計は17商品・665個とする。各商品について `restock` の在庫イベントを残す。
...
timezone
register_1_presale_enabled
register_2_presale_enabled
register_3_presale_enabled
```
...
通常販売UIは次の会計前注文APIを優先し、`POST /api/sales` は前売り販売と互換経路として維持します。

* `POST /api/staff/register/checkout-drafts`: 商品確定、注文番号発行、3分間の在庫仮押さえ
* `POST /api/staff/register/checkout-drafts/:draftId/complete`: 支払確認と売上への一括変換
* `POST /api/staff/register/checkout-drafts/:draftId/cancel`: 未会計注文の取消と仮押さえ解放
...
* おつりもサーバー側で再計算する
* 在庫不足なら409
* 商品別の前売り上限を超える場合は `PRESALE_LIMIT_EXCEEDED` の409とし、注文全体を更新しない
...
### 16.10a GET / POST `/api/staff/stock/noon-restock`
...
GETは本日の12時一括補充の内容と実施状態を返します。POSTは確認済みの手動操作として17商品・合計665個を一括補充します。`stock_restock_batches` の `(batch_key, business_date)` UNIQUE制約により、同じ営業日の二重補充を防止してください。在庫更新、17件の在庫イベント、実施記録は同一のD1 batchで確定してください。
